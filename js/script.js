// js/script.js — mobilní menu: spolehlivé klikání + touch fallback
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Odkazy v menu: zavřít menu a nechat přirozenou navigaci; robustní touch fallback
  if (navLinks) {
    navLinks.querySelectorAll('a[href]').forEach(a => {
      a.addEventListener('click', () => { closeMenu(); }, { passive: true });

      a.addEventListener('touchend', (ev) => {
        // Některé prohlížeče na mobilech doručí jen touch, bez clicku
        if (navLinks.classList.contains('active')) {
          const url = a.getAttribute('href');
          closeMenu();
          setTimeout(() => { window.location.assign(url); }, 10);
        }
      }, { passive: true });
    });
  }
  ...
  window.addEventListener('resize', () => {
    if (navLinks && navLinks.classList.contains('active')) {
      document.body.classList.add('menu-open');
      if (mainContent) mainContent.classList.add('menu-open');
      if (navBackdrop) { navBackdrop.style.opacity = '1'; navBackdrop.style.visibility = 'visible'; }
    } else {
      document.body.classList.remove('menu-open');
      if (mainContent) mainContent.classList.remove('menu-open');
      if (navBackdrop) { navBackdrop.style.opacity = '0'; navBackdrop.style.visibility = 'hidden'; }
    }
  }, { passive: true });
